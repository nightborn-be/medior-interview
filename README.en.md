...
> **Just arrived?** The exercise is in **[`EXERCISE.md`](EXERCISE.md)**. Read that first, then
> the client call notes in [`CLIENT-FACTS.md`](CLIENT-FACTS.md). Your answers go in the
> [`answers/`](answers/) folder, whose skeletons are already in place.
...
If a setup problem blocks you for more than ten minutes, write it down in your submission and
work around it.
...
| Path | Contents |
| --- | --- |
| `EXERCISE.md` | The exercise |
| `CLIENT-FACTS.md` | Notes from the scoping call with the client |
| `answers/` | The files you fill in |
...
| `docs/erp-import-spec.en.md` | Specification of the ERP import folder |
...
The French versions of the documents sit next to the English ones: `ENONCE.md`,
`FAITS-CLIENT.md`, `rendu/`, `docs/erp-import-spec.md`, `docs/annexe-plan-exemple.md`. Answer in
whichever language you prefer, and only fill in one of the two answer folders.

## Data

`data/catalog.csv`: `sku_code`, `description_nl`, `description_fr`, `packaging`, `unit`,
`category`. Comma separated, UTF-8. The descriptions are in Dutch and French because that is
what the customers write. They are not translated: that multilingual catalogue is the actual
difficulty of the case.

`data/lignes-transcrites.json`: the lines of the 8 orders, as written, with the customer code
and the delivery mention when there is one. No match against the catalogue has been made. The
field names are French: `commandes` are the orders, `lignes` the order lines, `client` the
customer name, `langue` the language, `fichier_source` the original file, `mention_livraison`
any delivery date the customer mentioned, and `recu_le` the time it arrived.
...
`docs/erp-import-spec.en.md`. It watches a drop folder, validates the files it finds there,
writes accepted lines into the `erp_orders` table, moves accepted files to `erp/processed/` and
rejected files to `erp/rejected/` along with a rejection log.
...
The schedule comes from `ERP_IMPORT_CRON` in `.env` (default: `0 23 * * *`). The folder paths
come from `ERP_DROP_DIR`, `ERP_PROCESSED_DIR`, `ERP_REJECTED_DIR` and `ERP_LOG_DIR`.
